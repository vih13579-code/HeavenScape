        ...
        <body class="bg-background text-on-background min-h-screen flex flex-col">
            <header class="w-full px-4 md:px-margin-desktop h-16 flex items-center justify-between bg-transparent">
                <div class="font-bold text-primary">
                    <img src="${pageContext.request.contextPath}/assets/images/logo/logoHS_2.png" alt="HeavenScape Logo"
                        class="w-[220px] mb-3" />
                </div>
            ...
            <main class="flex-grow flex items-center justify-center px-margin-mobile py-stack-lg">
                <div class="w-full max-w-md animate-fade-in">
                    <div
                        class="bg-surface border border-outline-variant/30 rounded-xl p-stack-lg shadow-[0_8px_30px_rgb(21,101,192,0.08)]">

                        <div class="flex justify-center mb-stack-md">
                            <div
                                class="w-20 h-20 bg-primary-container rounded-full flex items-center justify-center shadow-inner">
                        ...
                        <c:if test="${not empty sessionScope.otp_error}">
                            <div
                                class="mb-4 p-3 bg-error-container text-on-error-container border-l-4 border-error font-body-sm text-body-sm rounded-lg flex items-center gap-2">
                        ...
                        <c:if test="${not empty sessionScope.otp_success}">
                            <div
                                class="mb-4 p-3 bg-success-container text-on-success-container border-l-4 border-[#2E7D32] font-body-sm text-body-sm rounded-lg flex items-center gap-2">
                                <span class="material-symbols-outlined text-[18px]"
                                    style="color:#2E7D32">check_circle</span>
                                <span>${sessionScope.otp_success}</span>
                            </div>
                            <% session.removeAttribute("otp_success");%>
                        </c:if>

                        <form class="space-y-stack-lg" id="otp-form" action="${pageContext.request.contextPath}/otp"
                            method="POST">
                            ...
                            <div class="space-y-stack-sm">
                                ...
                                <div class="text-center py-base">
                                    <p class="font-body-sm text-body-sm text-on-surface-variant" id="countdown-wrapper">
                                        Please wait <span class="text-primary font-semibold"
                                            id="countdown-timer">05:00</span> before requesting another code.
                                    </p>
                                    <p class="font-body-sm text-body-sm text-on-surface-variant hidden"
                                        id="resend-wrapper">
                        ...
                        <form id="resend-form" action="${pageContext.request.contextPath}/otp" method="POST"
                            class="hidden">
                            ...
                            <c:choose>
                                <c:when test="${sessionScope.otp_flow == 'change_email'}">
                                    <a href="${pageContext.request.contextPath}/profile"
                                        class="text-primary hover:underline">← Back to Profile</a>
                                </c:when>
                                <c:when test="${sessionScope.otp_flow == 'change_password'}">
                                    <a href="${pageContext.request.contextPath}/profile/change-password"
                                        class="text-primary hover:underline">← Back to Change Password</a>
                                </c:when>
                                <c:when test="${sessionScope.otp_flow == 'forgot'}">
                                    <a href="${pageContext.request.contextPath}/forgot-password"
                                        class="text-primary hover:underline">← Back to Forgot Password</a>
                                </c:when>
                                <c:otherwise>
                                    <a href="${pageContext.request.contextPath}/register"
                                        class="text-primary hover:underline">← Back to Registration</a>
                                </c:otherwise>
                            </c:choose>
                        </p>
                    </div>
            ...
            <footer
                class="flex flex-col md:flex-row justify-between items-center w-full px-4 md:px-margin-desktop py-stack-md gap-4 bg-surface-container-low mt-auto">
                <img src="${pageContext.request.contextPath}/assets/images/logo/logoHS_2.png" alt="HeavenScape Logo"
                    class="w-[220px] mb-3" />
                <div class="flex gap-6 text-on-surface-variant">
                    ...
                    <a class="font-body-sm text-body-sm hover:text-primary transition-colors" href="#">Privacy
                        Policy</a>
                    <a class="font-body-sm text-body-sm hover:text-primary transition-colors" href="#">Contact</a>
                </div>
                <div class="font-label-sm text-label-sm text-on-surface-variant">© 2026 HeavenScape. All rights
                    reserved.</div>
            </footer>
            <script>
