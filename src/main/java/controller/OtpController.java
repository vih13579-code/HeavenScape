...
//        của đăng ký
    ...
    private void handleChangeEmailSuccess(HttpServletRequest request, HttpServletResponse response,
        ...
        // Dữ liệu phiên bị thiếu/hỏng thì không thực hiện, tránh NPE hoặc cập nhật sai users
    ...
    private void handleResend(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            session.setAttribute("otp_success", "A new OTP was sent to your email.");
        } catch (Exception e) {
            e.printStackTrace();
            session.setAttribute("otp_error", "Could not send the email. Please try again.");
        }
