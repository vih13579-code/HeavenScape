...
public class AccountManagementController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        int customerOffset = Math.max(0, Math.min(offset, totalCustomers));
        int customerLimit = Math.max(0, Math.min(pageSize, totalCustomers - customerOffset));
        ...
        if (queryCustomers) {
            if (customerLimit > 0) {
                ...
                request.setAttribute("customers", java.util.Collections.emptyList());
            }
        }

        int staffOffset = Math.max(0, offset - totalCustomers);
        int staffLimit = pageSize - customerLimit;
        ...
        if (queryStaffs) {
            if (staffLimit > 0) {
                ...
                request.setAttribute("staffs", java.util.Collections.emptyList());
            }
        }
